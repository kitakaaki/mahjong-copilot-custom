...
class LanStr:
    ...
    ENABLE_AUTO_NO_CHIPONKANG = "Enable Auto No Chi/Pon/Kan (鳴なし)"
    AUTO_CONFIRM_ROUND_END = "Auto Confirm Round End"
    AUTO_CONFIRM_ROUND_END_DURATION = "Round End Confirm Duration (sec)"
    ...
    AUTO_RANDOM_LEVEL_BY_PROB = "Auto Pick AI Level by Probability"
    AI_LEVEL_PROBABILITIES = "AI Level Probabilities (%)"
    AI_LEVEL_PROBABILITY_ERROR = "Set 6 non-negative level probabilities with total greater than 0"
...
class LanStrZHS(LanStr):
    ...
    ENABLE_AUTO_NO_CHIPONKANG = "启用自动不吃碰杠(鸣牌关闭)"
    AUTO_CONFIRM_ROUND_END = "局结束自动确认"
    AUTO_CONFIRM_ROUND_END_DURATION = "局结束自动确认时长(秒)"
    DRAG_DAHAI = "鼠标拖拽出牌"
    RANDOM_CHOICE = "AI 选项随机化(去重)"
    AUTO_ADJUST_RANDOM_LEVEL = "按对局结果自动调整 AI 随机等级"
    AUTO_RANDOM_LEVEL_BY_PROB = "按概率自动选择 AI 等级"
    AI_LEVEL_PROBABILITIES = "AI 等级概率(%)"
    AI_LEVEL_PROBABILITY_ERROR = "请设置 6 个非负等级概率，且总和大于 0"
...
class LanStrJA(LanStr):
    ...
    ENABLE_AUTO_NO_CHIPONKANG = "鳴きなし(チー/ポン/カンなし)を自動有効化"
    AUTO_CONFIRM_ROUND_END = "局終了の確認を自動クリック"
    AUTO_CONFIRM_ROUND_END_DURATION = "局終了確認の継続時間(秒)"
    DRAG_DAHAI = "ドラッグで打牌"
    RANDOM_CHOICE = "AI選択をランダム化"
    AUTO_ADJUST_RANDOM_LEVEL = "対局結果でAIランダムレベルを自動調整"
    AUTO_RANDOM_LEVEL_BY_PROB = "確率でAIレベルを自動選択"
    AI_LEVEL_PROBABILITIES = "AIレベル確率(%)"
    AI_LEVEL_PROBABILITY_ERROR = "6つの非負の確率を設定し、合計を0より大きくしてください"
